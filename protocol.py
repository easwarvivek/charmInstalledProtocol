...
from charm.core.engine.recvall import *
from charm.core.engine.send_recv import *
...
#            self.trans_map = trans_map
    ...
    def send_msg(self, object):
        ...
        if self._socket != None:
            if self._serialize:
                #print("DEBUG: serializing the object in send_msg")
                result = self._user_serialize(object)
                print("sending data of size: ", sys.getsizeof(result))
                #result = self.serialize(object)
                #print("DEBUG: sending via socket using send_msg")
                #self._socket.send(result)
                send_msg(self._socket,result)
            else:
                result = self.serialize(object)
                print("sending data of size: ", sys.getsizeof(result))
    ...
    def recv_msg(self):
        ...
        if self._socket != None:
            ...
            result = recv_msg(self._socket)
            print("received type:", type(result))
            print("received size:",sys.getsizeof(result))
            ...
            if result == '':
                ...
                if self._serialize:
                    #print("deserialize received object")
...
#        print("deserialize input =>", bytes_object)
        if type(bytes_object) == bytes:
            ...
            #print("DEBUG: unpickled the object")
            #print(object)
            if isinstance(object, dict):
                #print("DEBUG: unpickled object is a dict")
